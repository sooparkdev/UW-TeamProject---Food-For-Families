...
router.post('/import', async (req, res) => {
    try {
        ...
        if (Object.keys(foodResources).length === 0 || foodResources === null) {
            res.status(400).send({
                "status": "error",
                "error": "No JSON data to import"
            })
        } else {
            ...
            foodResources.forEach(async (foodResource) => {
                try {
                    ...
                    let frLatitude = Number(foodResource.latitude)
